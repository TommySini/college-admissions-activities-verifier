...
export const NavItemBase = ({ current, type, badge, href, icon: Icon, children, truncate = true, onClick }: NavItemBaseProps) => {
    const iconElement = Icon && <Icon aria-hidden="true" className="mr-2 size-5 shrink-0 text-black transition-inherit-all" />;
    ...
    const labelElement = (
        <span
            className={cx(
                "flex-1 text-md font-semibold text-black transition-inherit-all group-hover:text-gray-900",
                truncate && "truncate",
    ...
    const externalIcon = isExternal && <Share04 className="size-4 stroke-[2.5px] text-black" />;
    ...
    if (type === "collapsible") {
        return (
            <summary className={cx("px-3 py-2", styles.root, current && styles.rootSelected)} onClick={onClick}>
                ...
                <ChevronDown aria-hidden="true" className="ml-3 size-4 shrink-0 stroke-[2.5px] text-black in-open:-scale-y-100" />
            </summary>
        );
    }
