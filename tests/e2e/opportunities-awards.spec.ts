...
test.describe('E2E: Opportunity Awards Display', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page
      .waitForSelector('[data-testid="opportunity-card"]', {
        timeout: 10000,
        state: 'visible',
      })
      .catch(() => {
  ...
  test('page loads without crashing', async ({ page }) => {
    ...
    const cards = page
      .locator('.rounded-2xl')
      .filter({ hasText: /competition|program|scholarship/i });
  ...
  test('no console errors related to .map or awardTypes', async ({ page }) => {
    ...
    const mapErrors = consoleErrors.filter(
      (err) => err.includes('.map') || err.includes('awardTypes')
    );
  ...
  test('opportunity with awards shows badges', async ({ page }) => {
    ...
    if (await cardWithAwards.isVisible()) {
      ...
      const hasAwardsSection = await cardWithAwards
        .locator('text="Awards"')
        .isVisible()
        .catch(() => false);
      ...
      if (hasAwardsSection) {
        ...
        const badges = cardWithAwards
          .locator('.text-xs')
          .filter({ hasText: /cash|scholarship|recognition|publication/i });
  ...
  test('opportunity with no awards does not crash', async ({ page }) => {
    ...
    if (await cardNoAwards.isVisible()) {
      ...
      const hasAwardsSection = await cardNoAwards
        .locator('text="Awards"')
        .isVisible()
        .catch(() => false);
