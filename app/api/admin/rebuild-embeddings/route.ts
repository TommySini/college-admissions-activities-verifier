...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[POST /api/admin/rebuild-embeddings] Starting rebuild for: ${modelsToIndex.join(", ")}`);
    ...
    const indexingPromise = (async () => {
      ...
      for (const modelName of modelsToIndex) {
        try {
          ...
          console.log(`[rebuild-embeddings] ✓ ${modelName}: ${result.totalIndexed} indexed, ${result.totalFailed} failed`);
        } catch (error) {
