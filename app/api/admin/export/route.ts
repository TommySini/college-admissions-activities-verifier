  ...
  try {
    ...
    students.forEach((student) => {
      if (student.activities.length === 0) {
        ...
        csvRows.push(`"${student.name}","${student.email}","","","","","","","","","",""`);
      } else {
        student.activities.forEach((activity) => {
          const verificationStatus = activity.verification?.status || activity.status || 'pending';
          const verifierName = activity.verification?.verifier?.name || '';
          const verifierEmail = activity.verification?.verifier?.email || '';
