...
function SignInContent() {
  ...
  const handleGoogleSignIn = async (role: 'student' | 'admin') => {
    ...
    try {
      ...
      const callbackUrl =
        finalRole === 'admin' ? `/admin?role=${finalRole}` : `/activities?role=${finalRole}`;
      ...
      setError(
        `An error occurred: ${err?.message || 'Please check the browser console and ensure Google OAuth is configured.'}`
      );
      setIsLoading(false);
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center p-4">
        <div className="bg-black/5 backdrop-blur-xl rounded-2xl p-8 max-w-md w-full shadow-2xl border border-black/10">
          <div className="text-center mb-8">
            ...
            <h2 className="text-xl text-black mb-4">Activity Verification Platform</h2>
            <p className="text-sm text-black/60">Choose your role and sign in with Google</p>
          </div>
