    ...
    <div className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center p-4">
        <div className="bg-black/5 backdrop-blur-xl rounded-2xl p-8 max-w-md w-full shadow-2xl border border-black/10 text-center">
          <div className="mb-6">
            <h1 className="text-3xl font-bold text-black mb-2">Authentication Error</h1>
            <div className="w-24 h-1 bg-yellow-500 mx-auto mb-4"></div>
          </div>

          <div className="mb-6 p-4 bg-red-100/90 backdrop-blur-sm rounded-lg border border-red-200">
            <p className="text-red-800 font-medium">{getErrorMessage(error)}</p>
            {error && <p className="text-sm text-red-600 mt-2">Error code: {error}</p>}
          </div>
