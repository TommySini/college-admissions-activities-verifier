...
export function OpportunitiesListing() {
  ...
  const fetchOpportunities = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/opportunities?${searchParamsString}`, {
        cache: 'no-store',
      });
      if (!response.ok) {
      ...
      console.warn('Falling back to mock opportunities due to fetch error:', error);
      const fallback = getMockOpportunities(currentFilters);
      setEditions(fallback.editions);
      setTotal(fallback.total);
    } finally {
      setLoading(false);
    }
  }, [currentFilters, searchParamsString]);
  ...
  const handleExportCalendar = (edition: any) => {
    ...
    downloadICalFile({
      ...
      startDate: edition.eventStart
        ? new Date(edition.eventStart)
        : new Date(edition.registrationDeadline),
      endDate: edition.eventEnd
        ? new Date(edition.eventEnd)
        : new Date(edition.registrationDeadline),
  ...
  const quickFilters = [
    {
      ...
      onClick: () =>
        updateFilters({ popular: currentFilters.popular === 'true' ? undefined : 'true' }),
    },
    {
      label: '🎯 Open Now',
      active: currentFilters.status === 'open',
      onClick: () =>
        updateFilters({ status: currentFilters.status === 'open' ? undefined : 'open' }),
    },
    {
      label: '💰 Free',
    ...
    },
    {
      ...
      onClick: () =>
        updateFilters({ modality: currentFilters.modality === 'online' ? undefined : 'online' }),
    },
    {
      label: '👥 Team',
      active: currentFilters.structure === 'team',
      onClick: () =>
        updateFilters({ structure: currentFilters.structure === 'team' ? undefined : 'team' }),
    },
  ];
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <Link
                href="/opportunities/petition/new"
            ...
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                ...
                <p className="mt-3 text-slate-700 dark:text-slate-300 max-w-2xl">
                  Find competitions, programs, scholarships, internships, and more opportunities
                  curated for high school students.
                </p>
              </div>
